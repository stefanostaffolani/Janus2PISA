...
public class FromUntilTest {
  ...
  void setUp() throws Exception {
    InputStream is = FromUntilTest.class.getResourceAsStream("/loop.janus");
  ...
  @Test
  void testBranchNE() throws IOException {
    long bneCount =
        this.code.instructions().stream()
            .map(li -> li.instruction())
  ...
  void testLabels() {
    ...
    long nEntry = lis.stream().filter(li -> li.label().contains("entry")).count();
    assertEquals(1, nEntry);
    ...
    long nExit = lis.stream().filter(li -> li.label().contains("exit")).count();
    assertEquals(1, nExit);
  }
}
