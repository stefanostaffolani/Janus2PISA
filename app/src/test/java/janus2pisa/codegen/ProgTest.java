...
public class ProgTest {
  ...
  void testLabels() {
    ...
    long nStart = lis.stream().filter(li -> li.label().matches("^start$")).count();
    assertEquals(1, nStart);
    long nFinish = lis.stream().filter(li -> li.label().matches("^finish$")).count();
    assertEquals(1, nFinish);
  }
}
